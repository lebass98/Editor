...
export default function AnimatedArticleContent({ data }: { data: any }) {
    // If we have content (markdown), we display it instead of hardcoded body1-body7
    const hasMarkdownContent = !!data.content;
    ...
    return (
        <div className="w-full lg:w-1/2 bg-[#fbfcf6] text-black relative z-10 flex flex-col shadow-[-10px_0_30px_rgba(0,0,0,0.05)]">
            ...
            >
                ...
                <h1 className="text-4xl sm:text-5xl lg:text-[5vw] font-bold leading-[1.5] lg:leading-[1.15] tracking-tight text-white lg:text-black mb-8 break-keep word-break-keep-all pr-5">
                    <span className="underline decoration-[4px] lg:decoration-[.4vw] underline-offset-[5px] lg:underline-offset-[.8vw] decoration-white lg:decoration-black">
                        {data.titleLines ? 
                            ...
                            (typeof data.title === 'string' ? data.title : [data.title.line1, data.title.line2, data.title.line3].filter(Boolean).join(' '))
                        }
                    </span>
                </h1>
                ...
                <div className="flex items-center gap-4 text-[13px] font-bold text-white/80 lg:text-black">
                    <span>{data.date}</span>
                    <span className="flex items-center gap-1">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        {data.readingTime}분 읽기
                    </span>
                </div>
            ...
            <div className="mt-12 pb-16 text-black text-[16px] leading-[1.6] tracking-tight break-keep flex flex-col">
                {hasMarkdownContent ? (
                    <div className="px-3 lg:px-[30px] xl:px-[30px] markdown-content">
                        {/* We preserve the layout parts as much as possible */}
                        <motion.div
                    ...
                    <>
                        ...
                        >
                            <Image src={getImagePath(data.figureImage)} alt={typeof data.title === 'string' ? data.title : data.title.line1} width={800} height={600} className="w-full h-auto object-contain" />
                        </motion.figure>
                ...
                >
                    ...
                    <div className="flex items-start gap-5">
                        <div className="w-16 h-16 rounded-full border border-zinc-200 shrink-0 flex items-center justify-center overflow-hidden bg-white">
                            <Image src={getImagePath(data.authorImg)} alt={data.author} width={40} height={40} className="object-contain" />
                        </div>
                        <div className="flex flex-col">
                            <div className="font-bold text-[15px] mb-2 text-black">{data.author}</div>
