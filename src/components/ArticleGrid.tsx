...
export default function ArticleGrid({
  ...
  const textColor = theme === 'black' ? 'text-white' : 'text-black dark:text-white';
  const subTextColor = theme === 'black' ? 'text-zinc-400' : 'text-zinc-600 dark:text-zinc-400';
    ...
    return (
      ...
      >
        <Link href={`/article/${article.id}`} className={`group flex ${isReversed ? 'flex-row-reverse' : 'flex-row'} md:flex-col gap-3 md:gap-0 h-auto md:h-full bg-transparent items-start md:items-stretch`}>
          <div className="relative overflow-hidden aspect-[4/5] object-cover bg-zinc-100 dark:bg-zinc-800 w-1/2 md:w-full shrink-0 md:mb-4">
            <Image
          ...
          <div className="flex flex-col flex-1 py-1 md:py-0">
            ...
            <div className={`flex items-center justify-between border-t md:border-t-0 border-black/10 dark:border-white/10 md:relative ${textColor} mt-auto pt-4 md:pt-0`}>
              <span className="text-[12px] md:text-[13px] font-medium opacity-60 md:opacity-100">{article.date}</span>
              <div className="flex flex-col items-center">
                <div className="w-8 h-8 md:w-9 md:h-9 rounded-full overflow-hidden mb-1 shrink-0 border border-black/10 dark:border-white/10 shadow-sm bg-white dark:bg-zinc-800">
                  <Image src={getImagePath(authorData.image)} alt={authorData.name} width={36} height={36} className="object-cover" />
                </div>
                <span className="text-[10px] md:text-[11px] font-bold">{authorData.name}</span>
              </div>
            </div>
          </div>
        </Link>
